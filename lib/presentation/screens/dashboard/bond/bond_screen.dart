...
class _BondScreenState extends State<BondScreen> {
  ...
  Future<void> getUserProfiles() async {
    ...
    // Fallback to placeholder match if no match or match data is invalid
    final placeholderSnapshot =
        await db.collection("users").doc(placeholder_match).get();
    final placeholderUser = AppUser.fromSnapshot(placeholderSnapshot);

    setState(() {
      match = placeholderUser;
      keepMatchToggle = false;
      isMatchBlocked = false;
      isMatchUnbonded = false;
