...
class _MoreProfileScreenState extends State<MoreProfileScreen> {
  ...
  Future<void> fetchVisibilitySettings() async {
    ...
    setState(() {
      visibilityPrefs = {
        'showAge': data[widget.isMatchViewer ? 'showAgeToMatch' : 'showAgeToOthers'] ?? true,
        'showMajor': data[widget.isMatchViewer ? 'showMajorToMatch' : 'showMajorToOthers'] ?? true,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("${widget.name}'s Profile", style: const TextStyle(color: Color(0xFF5E77DF))),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (visibilityPrefs['showSocialMedia']!)
                      Column(
                        children: [
                          _buildSocialMediaRow("Instagram", Icons.camera_alt, ""),
                          _buildSocialMediaRow("Facebook", Icons.facebook, ""),
                          if (widget.spotifyUsername != null &&
                              widget.spotifyUsername!.isNotEmpty)
                            _buildSocialMediaRow(
                                "Spotify", Icons.music_note, "https://open.spotify.com/user/${widget.spotifyUsername}"),
                        ],
                      ),

                    const SizedBox(height: 20),
                    _buildProfileField("Name", widget.name),
                    if (visibilityPrefs['showAge']!) _buildProfileField("Age", widget.age),
                    if (visibilityPrefs['showMajor']!) _buildProfileField("Major", widget.major),
                    if (widget.showHeight && visibilityPrefs['showHeight']!)
                      _buildProfileField("Height", _getFormattedHeight()),
                    if (visibilityPrefs['showBio']!) _buildProfileField("Bio", widget.bio),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(label,
              style: TextStyle(
                  fontSize: 14, fontWeight: FontWeight.w300, color: Colors.grey[600])),
  ...
  Widget _buildSocialMediaRow(String label, IconData icon, String url) {
    return Padding(
      ...
      child: OutlinedButton.icon(
        ...
        label: Text(label,
            style: TextStyle(
                fontSize: 16, color: url.isNotEmpty ? Colors.blueAccent : Colors.grey)),
  ...
  Widget _buildPhotos(List<String> photosURL) {
    return Column(
      children: [
        Text("Photos",
            style: TextStyle(
